...
remove(list=ls())
getwd()
setwd('C:/Users/Marcus/Box\ Sync/Courses/Quantitative\ Biodiversity/QB2019_Hibbins/2.Worksheets/3.RStudio/')
...
```{r}
column1 <- c(rnorm(5, mean = 8, sd = 2))
column2 <- c(rnorm(5, mean = 25, sd = 10))
columns <- c(column1, column2)
matrix(data = columns, nrow = 5, ncol = 2, byrow = FALSE)
...
> Answer 1: This function randomly samples the specified number of observations from a normal distribution with the specified mean and standard deviation. 
...
> Answer 3: Many variables appear to be highly correlated, with coefficients in the 60-95% range. There are also quite a few appreciable, but not huge, coefficients in the 10-30$ range. Some particular variable combinations have a low correlation coefficient. As far as signs go, the variable CHLA appears to be negatively correlated with all the other variables (to varying degrees), while all the other coefficients are positive. 
...
> Answer 4: Using a non-parametric method does result in different coefficient estimates and p-values, but in this particular case, this effect does not appear to change our conclusions about the data. For Pearson's method, there is evidence for a false discovery rate, because the adjusted p-values above the diagonal are higher than the uncorrected p-values. P-values represent the probability that the data were generated under a given null hypothesis. Typically we reject the null for p-values below 0.05, but under this standard we will reject 5% of all datasets generated under the null hypothesis - these are false positives. For large datasets, 5% of results can represent a large number of p-values, so adjusting for this false positive rate is crucial. 
...
```{r}
zoo_reg <- lm(ZP ~ TN, data = meso)
summary(zoo_reg)
require(ggplot2)
lm_plot <- ggplot(meso, aes(x = TN, y = ZP))
lm_plot <- lm_plot + geom_point()
lm_plot <- lm_plot + geom_smooth(method=lm)
lm_plot <- lm_plot + geom_text(aes(label=NUTS), hjust=2)
lm_plot <- lm_plot + labs(x = expression(paste('Total Nitrogen (', mu, 'g/L)')),
                          y = 'Zooplankton Biomass (mg/L)')
lm_plot
...
> Answer 5: The slope of the relationship between TN and ZP is significantly (p = 0.00001911) different from 0. The estimate of 0.0013 means that for every 1 ug/L increase in total nitrogen, the zooplankton biomass increases by 0.001 mg/L. Based on the plots below, there do not appear to be any serious violations of the assumptions of linear regression. 
...
```{r}
meso$NUTS <- factor(meso$NUTS, levels = c('L', 'M', 'H'))
...
nut_barplot <- ggplot(meso, aes(x = NUTS, y = ZP))
nut_barplot <- nut_barplot + stat_summary(fun.y = mean, geom = 'bar')
nut_barplot <- nut_barplot + stat_summary(fun.data = sem, 
                                          geom = 'errorbar',
                                          width = 0.1)
nut_barplot <- nut_barplot + labs(x = 'Nutrient supply',
                                  y = 'Zooplankton biomass (mg/L)')
nut_barplot
mesoanova <- aov(ZP ~ NUTS, data = meso)
summary(mesoanova)
...
> Answer 6: Based on the barplot and ANOVA analysis presented below, it seems likely that the trends in total biomass are driven mostly by the CHYD variable, whuch are Chydorus zooplankton. In fact, the mean biomass of this taxonomic group is not significantly different (p = 0.457) from the mean total biomass across all species. 

```{r}
zoops <- read.table('./data/zoops.txt', header = TRUE)
zoops <- zoops [,c(3:11)]
zoops$total <- rowSums(zoops)
require(reshape2)
zoops.long <- melt(zoops)

zoops_barplot <- ggplot(zoops.long, aes(x = variable, y = value))
zoops_barplot <- zoops_barplot + stat_summary(fun.y = mean, geom = 'bar')
zoops_barplot <- zoops_barplot + stat_summary(fun.data = sem, 
                                          geom = 'errorbar',
                                          width = 0.1)
zoops_barplot <- zoops_barplot + labs(x = 'Taxon',
                                  y = 'Zooplankton biomass (mg/L)')
zoops_barplot

zoopsanova <- aov(value ~ variable, data = zoops.long)
summary(zoopsanova)
TukeyHSD(zoopsanova)
```
